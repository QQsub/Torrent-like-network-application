...
# Define a callback function to ensure session state keys are initialized

def initialize_session_state():
        st.session_state.is_running = False  # Tracker not running by default
        st.session_state.tracker_thread = None  # No tracker thread initially
# End Front End
...
def parse_torrent_file(torrent_file_path):
    try:
        ...
        st.write(f"Error parsing torrent file: {e}")
        return None
...
def parse_announce_data(data):
    ...
    try:
        ...
        st.write(f"Error parsing announce data: {e}")
        return None
...
def handle_announce(conn, data, addr):
    ...
    if tracker_id:
        ...
        st.write(f"Received tracker_id from peer: {tracker_id}")
    ...
    if event == "started":
        ...
        st.write(f"Peer {peer_id} connected to tracker.")
        ...
        st.write(f"Peer {peer_id} start downloading {file_name}")
        if torrent_joined not in peers:
            peers[torrent_joined] = []
        ...
        for torrent in peers.keys():
            for i in peers[torrent]:
                if i["peer_id"] == peer_id:
                    ...
                    st.write(f"Peer {peer_id} stopped and quit the torrent: {torrent}.")
        ...
        st.write(f"Peer {peer_id} completed downloading/uploading {file_name}.")
...
def start_tracker():
    ...
    server.listen(10)
    st.write(f"Tracker server started on port {TRACKER_PORT}")

    while st.session_state.get("is_running", False):
        try:
            conn, addr = server.accept()
            st.write(f"Connection from {addr}")
            data = conn.recv(1024)
            if data:
    ...
    server.close()
    st.write("Tracker server stopped.")
    
def manage_tracker():
    """Manage the tracker server state in a separate thread."""
    if st.session_state.is_running:
        st.session_state.tracker_thread = threading.Thread(target=start_tracker, daemon=True)
        st.session_state.tracker_thread.start()

# Streamlit UI
# Call the callback at the start of the app
def main():
    initialize_session_state()

    st.title("Tracker Server Manager")
    ...
    if st.button("Start Tracker"):
        if not st.session_state.is_running:
            ...
            st.warning("Tracker is already running.")

    if st.button("Stop Tracker"):
        if st.session_state.is_running:
            st.session_state.is_running = False
            if st.session_state.tracker_thread:
                st.session_state.tracker_thread.join()
            st.success("Tracker stopped.")
        else:
